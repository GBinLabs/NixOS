    ...
    security.sudo.extraRules = [
      {
        ...
        commands = [
          ...
          { command = "/run/current-system/sw/bin/tee /sys/class/drm/card*/device/power_dpm_force_performance_level"; options = [ "NOPASSWD" ]; }
          { command = "/run/current-system/sw/bin/tee /proc/sys/vm/drop_caches"; options = [ "NOPASSWD" ]; }
        ];
      }
    ];
